...
from src.adapters.gmail.interface import IGmailAdapter
from src.adapters.messaging.interface import IMessageBus
from src.adapters.storage.interface import IObjectStorage
from src.adapters.vault.interface import IVaultAdapter
from src.domain.attachment import Attachment
from src.domain.events import AttachmentReadyEvent, NewMailEvent
from src.domain.mail_message import MailMessage
from src.repositories.interface import IMailRepository
from src.services.message_parser import GmailMessageParser
...
class MailSyncService:
    ...
    3. For each message:
       ...
       e. Publish ``mail.new`` and ``attachment.ready`` events to Kafka.
    """
    ...
    async def _publish_new_mail_event(
        ...
        with self._logger.timed(
            "kafka.publish", topic="mail-events", event_type="mail.new"
        ):
    ...
    async def _publish_attachment_ready_event(
        ...
        with self._logger.timed(
            "kafka.publish", topic="attachment-events", event_type="attachment.ready"
        ):
