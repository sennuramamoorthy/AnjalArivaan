...
Run BEFORE implementing src/services/sync_service.py.
...
from domain.mail_message import MailMessage, UrgencyLevel
from domain.events import NewMailEvent, AttachmentReadyEvent

# Inline minimal logger for unit tests (avoids package install requirement)
import json
import sys as _sys
...
def sync_service(
    ...
    logger = get_logger("mail-sync-test", trace_id="trace_test_001")
...
class TestMailSyncService:
    ...
    async def test_sync_uploads_attachment_to_object_storage(
        ...
        from anjal_logger import get_logger  # type: ignore[import]
        logger = get_logger("mail-sync-test")
